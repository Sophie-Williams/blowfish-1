...
static const u32 g_p[18] = {
	0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0, 0x082EFA98, 0xEC4E6C89,
	0x452821E6, 0x38D01377, 0xBE5466CF, 0x34E90C6C, 0xC0AC29B7, 0xC97C50DD, 0x3F84D5B5, 0xB5470917,
	0x9216D5D9, 0x8979FB1B
};

static const u32 g_s[4][256] = { {
...
static u32 bf_f(bf_context *bf, u32 x) {
	u32 d = (x & 0xFF);
	x >>= 8;
	u32 c = (x & 0xFF);
	x >>= 8;
...
static void bf_encrypt_64bits(bf_context *bf, u32 *lhs_, u32 *rhs_) {
	u32 lhs = *lhs_;
	u32 rhs = *rhs_;
...
static void bf_decrypt_64bits(bf_context *bf, u32 *lhs_, u32 *rhs_) {
	u32 lhs = *lhs_;
	u32 rhs = *rhs_;
...
void bf_init(bf_context *bf, byte *key, size_t len) {
	assert((len >= 4) && (len <= 112));	

	memcpy(bf->s, g_s, sizeof(g_s));	
	for (u32 i = 0, j = 0; i < 18; ++i) {
		u32 data = 0;
		for (u32 k = 0; k < 4; ++k) {
			data = (data << 8) | key[j];
			++j;
